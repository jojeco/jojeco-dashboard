...
const STATE_LABELS: Record<string,string> = {
  downloading:'Downloading',uploading:'Seeding',pausedDL:'Paused',pausedUP:'Paused (done)',
...
type SortKey = 'progress' | 'name' | 'speed' | 'size';

function sortTorrents(list: Torrent[], key: SortKey): Torrent[] {
  const stateGroup = (s: string) => {
    if (s === 'downloading') return 0;
    if (['metaDL','allocating','stalledDL'].includes(s)) return 1;
    if (s === 'queuedDL') return 2;
    if (s.startsWith('paused') || s.startsWith('stopped')) return 3;
    if (['uploading','stalledUP','queuedUP','forcedUP'].includes(s)) return 4;
    return 5;
  };
  return [...list].sort((a, b) => {
    if (key === 'progress') {
      const ga = stateGroup(a.state), gb = stateGroup(b.state);
      if (ga !== gb) return ga - gb;
      return b.progress - a.progress;
    }
    if (key === 'speed') return b.dlspeed - a.dlspeed;
    ...
    <div className="p-4 md:p-6 space-y-4">
      {isGuest && (
        <div className="rounded-xl border border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-950/30 px-4 py-3 text-sm text-blue-700 dark:text-blue-300">
          <span className="font-semibold">Torrents</span> — active download queue managed by qBittorrent. Transfer speeds, progress, and queue management are visible to signed-in users.
        </div>
      )}
      {transfer && (
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {sel.length>0 && <>
            ...
            <button onClick={()=>{if(confirm(`Remove ${sel.length} torrent(s)?`)){act('delete',sel,{deleteFiles:false});setSelected(new Set());}}} className="flex items-center gap-1.5 px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm"><Trash2 className="w-4 h-4"/> Remove</button>
        ...
        <div className="flex items-center gap-1 overflow-x-auto scrollbar-none">
          <span className="text-xs text-gray-400 shrink-0 mr-1">Sort:</span>
          {(['progress','speed','size','name'] as SortKey[]).map(k=>(
            <button key={k} onClick={()=>setSortKey(k)}
              className={`shrink-0 px-2 py-1 rounded text-xs font-medium transition-colors ${sortKey===k ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
              {k}
            </button>
          ))}
        </div>
      ...
      {torrents.length===0 ? (
        ...
        <div className="space-y-2">
          {sortTorrents(torrents, sortKey).map(t=>(
            <div key={t.hash} onClick={()=>toggle(t.hash)}
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mb-2">
                <div className="h-1.5 rounded-full bg-blue-500" style={{width:`${(t.progress*100).toFixed(1)}%`}}/>
              </div>
              ...
              <div className="flex gap-2 mt-2 pt-2 border-t border-gray-100 dark:border-gray-700" onClick={e=>e.stopPropagation()}>
                ...
                <button onClick={()=>{if(confirm('Remove torrent?'))act('delete',[t.hash],{deleteFiles:false});}} className="flex items-center gap-1 px-2 py-1 text-xs bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-md ml-auto"><Trash2 className="w-3 h-3"/> Remove</button>
              </div>
            </div>
          ))}
        </div>
